...
for file in source_file:
 ...
 anyconnect = re.findall('AnyConnect Client *: *\d* : *\d* : *\d*.:.*\d', line_re)
 ssl_client = re.findall('Clientless VPN *: *\d* : *\d* : *\d*', line_re)
 ikev1 = re.findall('IKEv1 IPsec/L2TP IPsec \w* *: *\d* : *\d* : *\d*', line_re)
 conn = re.findall('Conns *\d* *\d* *\d* *\d* \w*', line_re)
 ...
 with open(name, 'a') as csvfile:
      ...
      filewriter.writerow(anyconnect)
      filewriter.writerow(ssl_client)
      filewriter.writerow(ikev1)
